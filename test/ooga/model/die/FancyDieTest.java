...
public class FancyDieTest {
  ...
  @Test
  void testDoubles() {
    initGamestate(new DoublesDie());

    Player p = myGameData.getCurrentPlayer();
    p.setLocation(0);
    myTurnManager.roll();
    assertNotEquals(p.getLocation(), 0);
  }

  @Test
  void testBowser() {
    initGamestate(new BowserDie());
